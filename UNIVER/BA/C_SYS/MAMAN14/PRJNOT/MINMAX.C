...

{
    ...
    if(depth==DEPTH_MIN_MAX-2)
	{
	ptr->son=generate(ptr->b->board,depth+1);
	free(ptr->b);
	}
}
/*******************************************************************/
...

{
    pointer best,           /* best move of the 1st level */
	    l1, l2, l3,     /* auxiliary variables for levels */
	    max, min;       /* hold maximum of 3rd and minimum of 4th levels */
    pointleave l4, m;
...
/*******************************************************************/
void *addnode(void *first, boardt brd, char level)
...

{
   ...
   if(level < DEPTH_MIN_MAX-1)
      {
      ptr=(pointer)malloc(sizeof(Tnode));      /* new node creating */
      ...
      {
      ptrl=(pointleave)malloc(sizeof(Leave));
      ptrl->val=0;
      ptrl->next=first;                        /* ptr is the first from now */
      first=ptrl;
      }
   return first;
}
...
{
    pointer l1=head->son, l2, l3,            /* auxiliary var's */
	    aux1, aux2, aux3;
    pointleave l4, aux4;
