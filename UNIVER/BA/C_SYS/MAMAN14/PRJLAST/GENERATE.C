...
pointer generate(boardt brd, char level)
...
/* becomes queen? */	     if(level < DEPTH_MIN_MAX-1)
				if((i==7 && sign>0) || (i==2 && sign<0))
				    {
				     first->b->board[i+sign][j+dir]=COMP_QUEEN*sign;
				     first->val+=sign*25;
				     }
/* near a boarder is safe */
			     if(j+dir==1 || j+dir==8) first->val+=sign;
			     }
...
/* forward is better than backward: */
				if(back*sign>0) first->val+=sign*2;
/* for all but the last level update the board: */
				if(level < DEPTH_MIN_MAX-1)
				   {
				   first->b->board[i][j]=EMPTY;
...
/* for all but the last level update the board: */
    ...
    if(pos_eat[0])   /* if eating has been */
	first=check_eat(first, brd, sign);
    return first;
}
